...
void Launch_Task_Init() {
    ...
    g_flywheel_left = DJI_Motor_Init(&flywheel_left_config,GM6020);
    g_flywheel_right = DJI_Motor_Init(&flywheel_right_config,GM6020);
...
void Feed_Angle_Calc()
{
    ...
    if (g_launch_target.single_launch_flag && !g_launch_target.single_launch_finished_flag) {
        g_launch_target.feed_angle = g_feed_angle->stats->total_angle_rad + FEED_1_PROJECTILE_ANGLE;
